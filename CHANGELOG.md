...
### Fixed
- Updated version number across all configuration files for consistency
- Fixed remaining pytest-asyncio warnings in test suite
- Improved error handling in API gateway initialization
- Updated LangChain deprecated imports (Chroma to langchain_chroma, HuggingFaceEmbeddings to langchain_huggingface)

## [2.6.5] - 2025-03-15

### Fixed
- Fixed FastAPI deprecation warning by replacing on_event handlers with lifespan context managers
- Improved API gateway health check task management with proper cleanup on shutdown

## [2.6.4] - 2025-03-14

### Fixed
- Fixed ModelRegistry._load_registry() error with duplicate model_id parameter
- Added missing system_check.py module referenced in the README
- Documented solution for Pydantic warnings about protected namespace "model_"
- Added missing get_llm_service_status function for diagnostics
- Fixed LangGraph compatibility by adding __call__ method to SupervisorAgent
- Fixed get_llm_service_status to use config instead of settings attributes
- Improved workflow diagnostics to gracefully handle LangGraph compatibility issues
- Enhanced diagnostic error handling for more graceful failure modes
